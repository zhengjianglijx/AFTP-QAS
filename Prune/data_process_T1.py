...
def load_dataset(name, embedding_type, num_reps, file_type='txt'):
    if file_type == 'txt':
        x_train = np.genfromtxt('../datasets/experiment_data/{}/x_train.txt'.format(name))
        x_test = np.genfromtxt('../datasets/experiment_data/{}/x_test.txt'.format(name))
        y_train = np.genfromtxt('../datasets/experiment_data/{}/y_train.txt'.format(name))
        y_test = np.genfromtxt('../datasets/experiment_data/{}/y_test.txt'.format(name))
    elif file_type == 'npy':
        x_train = np.load('../datasets/experiment_data/{}/x_train.npy'.format(name))
        x_test = np.load('../datasets/experiment_data/{}/x_test.npy'.format(name))
        y_train = np.load('../datasets/experiment_data/{}/y_train.npy'.format(name))
        y_test = np.load('../datasets/experiment_data/{}/y_test.npy'.format(name))
...
class TensorFlowDataset:
    def __init__(self, dataset_name, embed_type, reps, train=True, reshape_labels=False, file_type='txt'):
        ...
        if train:
            ...
            self.x_data = x_train[inds]
            self.y_data = y_train[inds]
        else:
            inds = np.random.permutation(len(x_test))

            self.x_data = x_test[inds]
            self.y_data = y_test[inds]
