...
class GatingNetwork(nn.Module):
    def __init__(self, arg):
        ...
        self.encoder.load_state_dict(
            torch.load(f'pretrained/{arg.task}_{arg.train_N_pretraining}/{arg.search_space}/model_7_{arg.loss_method}.pt')['encoder_state'])
    ...
    def __init__(self, arg):
        ...
        for i, expert in enumerate(self.experts, start=1):
            expert.load_state_dict(
                torch.load(f'pretrained/{arg.task}_{arg.train_N_pretraining}/{arg.search_space}/model_{i}_{arg.loss_method}.pt')['model_state'])
        self.expert_6.load_state_dict(
            torch.load(f'pretrained/{arg.task}_{arg.train_N_pretraining}/{arg.search_space}/model_6_{arg.loss_method}.pt')['model_state'])
...
class MoE(nn.Module):
    ...
    def forward_ablation(self, adj, ops, expert_to_remove):
        """
        Ablation experiment: The assigned experts are removed and the performance is evaluated.
        Parameters:
            -adj: Adjacency matrix of the graph
            -ops: The input to the operation
            -expert_to_remove: The index of experts to remove (0-5)
        Output:
            -output: The output of the model
            -gating_weights: The weights of the gated network
        """
        gating_weights = self.gating(adj, ops)
        # get the expert output, drop the specified expert
        expert_scores = self.experts(adj, ops)

        remaining_experts = [i for i in range(6) if i != expert_to_remove]
        expert_scores = expert_scores[:, remaining_experts]
        output = torch.sum(gating_weights * expert_scores, dim=1)
        return output, gating_weights
